...
1. **Application registers copy** by calling `copyService.registerCopy(jsonCopy)`. The JSON is deep-cloned and merged into `_registeredCopy`
2. **Application creates an evaluator** (e.g., `new PlainTextEvaluator(copyService)`)
3. **Application calls** `evaluator.getCopy('some.key', { value: 42 })`
4. `getCopy()` wraps raw substitutions in a `Substitutions` instance (enables lazy evaluation)
5. `getCopy()` calls `copyService.getAstForKey('some.key')`
6. **Lazy parse (first access only):** If the value at this key is still a string, `getAstForKey()` calls `Parser.parseSingle(key, rawString)` to tokenize and parse the DSL string into an AST. The AST replaces the string in `_registeredCopy`
7. The AST is returned to the evaluator
8. `getCopy()` calls `this.evalAST(this.getInitialResult(), ast, substitutions)`
9. **Evaluation walks the AST:** For each node, the evaluator checks the cache, evaluates the node (dispatching on `instanceof` type), recurses on `ast.sibling`, caches the result if cacheable, and returns the accumulated output
10. **Substitution nodes** read values from the `Substitutions` instance
11. **Reference nodes** call `copyService.getAstForKey(refKey)` recursively, which may trigger another lazy parse
